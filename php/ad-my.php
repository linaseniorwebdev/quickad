 ...
 WHERE status = 'active' AND hide = '0' AND user_id = '".$_SESSION['user']['id']."' ";
    ...
    if ($result) {
        foreach ($result as $info)
        {
            ...
            $items[$info['id']]['link'] = $config['site_url']. $info['id'] . '/'.'ikmanad/' .$pro_url;
            $items[$info['id']]['catlink'] = $config['site_url'].'ikman/'.$get_main['slug'];
            $items[$info['id']]['subcatlink'] = $config['site_url'].'ikman/'.$get_main['slug'].'/'.$get_sub['slug'];
