...
if ($num_rows > 0) {
    ...
    $item_catlink = $config['site_url'].'ikman/'.$get_main['slug'];
    $item_subcatlink = $config['site_url'].'ikman/'.$get_main['slug'].'/'.$get_sub['slug'];
    ...
    $item_link = $config['site_url']. $info['id'] . '/'.'ikmanad/' .$pro_url;
...
if (count($result1) > 0) {
    ...
    foreach ($result1 as $info1 ) {
        ...
        $item[$info1['id']]['link'] = $config['site_url']. $info1['id'] . '/'.'ikmanad/' .$pro_url;

        $cat_url = create_slug($get_main['cat_name']);
        $item[$info1['id']]['catlink'] = $config['site_url'].'ikman/'.$info1['category'].'/'.$cat_url;
