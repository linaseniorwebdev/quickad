...
function get_subcat_of_maincat($category_id,$adcount=false,$selected="",$selected_text='selected'){
    ...
    foreach($result as $info){
        ...
        $subcat[$info['sub_cat_id']]['link'] = $config['site_url'].'ikman/'.$category_slug.'/'.$subcat_slug;
...
$where ORDER BY $order_by $pagelimit";
    ...
    if ($result) {
        foreach ($result as $info)
        {
            ...
            $item[$info['id']]['link'] = $config['site_url']. $info['id'] . '/'.'ikmanad/' .$pro_url;
            $item[$info['id']]['catlink'] = $config['site_url'].'ikman/'.$get_main['slug'];
            $item[$info['id']]['subcatlink'] = $config['site_url'].'ikman/'.$get_main['slug'].'/'.$get_sub['slug'];
...
function get_resubmited_items($userid=false,$status=null,$page=null,$limit=null,$sort="id"){
    ...
    if ($result) {
        foreach ($result as $info)
        {
            ...
            $item[$info['id']]['catlink'] = $config['site_url'].'ikman/'.$catslug;

            $item[$info['id']]['subcatlink'] = $config['site_url'].'ikman/'.$catslug.'/'.$subcatslug;
